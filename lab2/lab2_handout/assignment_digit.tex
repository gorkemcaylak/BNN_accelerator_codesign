...
%\item $directive.tcl$: an empty Tcl script where you insert optimization directives in Tcl commands.
...
%\item $setup$: an \textbf{updated} bash script (different from Assignment 1) to set up Xilinx tools (for v2014.2) in your environment. You must either source this script manually immediately after each login (. ./$setup$) or add it to your $.bashrc$ file. Note that there are \textbf{two dots} separated by a space in the script sourcing command.
%\item $assignment\_cordic.pdf$: instructions for this assignment (this file).
\end{itemize}
